...
test('tennis ball is rendered via CSS without inline imagery', async ({ page }) => {
  await openRoulette(page);

  const childCount = await page.locator('.roulette__ball').evaluate((el) => el.childElementCount);
  expect(childCount).toBe(0);

  const backgroundImage = await page
    .locator('.roulette__ball')
...
test('pointer anchors above the wheel, dips into it, and stays centred', async ({ page }) => {
  await openRoulette(page);
  ...
  expect(pointerTipY).toBeGreaterThan(wheelBox.y + 6);
  expect(pointerTipY).toBeLessThan(wheelBox.y + wheelBox.height * 0.18);

  const pointerColor = await page
    .locator('.roulette__pointer')
    .evaluate((el) => getComputedStyle(el).borderTopColor);
  expect(pointerColor).toBe('rgb(255, 255, 255)');
});

test('start marker extends from the centre to the pointer and rotates with the wheel', async ({ page }) => {
  await openRoulette(page);

  const wheel = page.locator('.roulette__wheel');
  const marker = page.locator('.roulette__marker');
  ...
  if (!markerBox || !wheelBox) {
    throw new Error('Expected marker and wheel to have bounding boxes');
  }

  const markerBottomY = markerBox.y + markerBox.height;
  const wheelCentreY = wheelBox.y + wheelBox.height / 2;
  expect(Math.abs(markerBottomY - wheelCentreY)).toBeLessThan(2.5);

  const markerCentreX = markerBox.x + markerBox.width / 2;
  const wheelCentreX = wheelBox.x + wheelBox.width / 2;
  expect(Math.abs(markerCentreX - wheelCentreX)).toBeLessThan(2);
  ...
  const idleTop = await readMarkerTop();
  await page.evaluate(() => window.pointroulette.start());
  await page.waitForTimeout(420);
  const activeTop = await readMarkerTop();

  const deltaX = Math.abs(activeTop.x - idleTop.x);
  const deltaY = Math.abs(activeTop.y - idleTop.y);
  expect(Math.hypot(deltaX, deltaY)).toBeGreaterThan(8);
});
